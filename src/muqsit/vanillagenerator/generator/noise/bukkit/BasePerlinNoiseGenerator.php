...
abstract class BasePerlinNoiseGenerator extends NoiseGenerator{
	...
	public function __construct(?Random $rand = null){
		if($rand === null){
			...
			// Enhanced offset range for 1.18 terrain variety
			$this->offset_x = $rand->nextFloat() * 384;
			$this->offset_y = $rand->nextFloat() * 384;
			$this->offset_z = $rand->nextFloat() * 384;
	...
	public function noise3d(float $x, float $y = 0.0, float $z = 0.0) : float{
		...
		// Enhanced fade curves for smoother 1.18-like terrain
		$fX = self::fade($x);
		$fY = self::fade($y);
		$fZ = self::fade($z);
		...
		$result = self::lerp($fZ, self::lerp($fY, self::lerp($fX, self::grad($this->perm[$AA], $x, $y, $z),
		...
		// Slight amplification for 1.18 terrain characteristics
		return $result * 1.15;
	}
}

// Enhanced OctaveGenerator for 1.18 terrain
abstract class OctaveGenerator extends BaseOctaveGenerator{

	/**
	 * Generates noise for the 3D coordinates using the specified number of
	 * octaves and parameters - Enhanced for 1.18 terrain
	 */
	public function noise(float $x, float $y, float $z, float $frequency, float $amplitude, bool $normalized) : float{
		...
		foreach($this->octaves as $octave){
			...
			$max += $amp;
			$freq *= $frequency;
			// Slightly adjusted amplitude decay for better 1.18 terrain
			$amp *= $amplitude * 0.95;
		}
