...
class PlainsTerrain implements BiomeTerrainGenerator {
     ...
     */
    private function calculatePlainsSurfaceHeight(
        int $x,
        int $z,
        array $density,
        int $min_y,
        ...
        // Basis-Höhe knapp über Meeresspiegel
        $base_height = $sea_level + 2;

        // Weniger Variation für flachere Ebenen
        $variation = (int)($surface_noise * 3);

        // Sanfte Hügel, breit verteilt
        $hill_factor = (int)((sin($x * 0.01) + cos($z * 0.01)) * 1.5);

        $height = $base_height + $variation + $hill_factor;

        // Clamp: nie unter Meeresspiegel, nicht zu hoch
