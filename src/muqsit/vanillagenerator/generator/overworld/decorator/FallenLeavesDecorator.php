...
class FallenLeavesDecorator extends Decorator{

	private const SOIL_TYPES = [BlockTypeIds::GRASS];

	public function decorate(ChunkManager $world, Random $random, int $chunk_x, int $chunk_z, Chunk $chunk) : void{
		if($random->nextBoundedInt(100) >= 100){
            return;
        }
		...
		for($i = 0; $i < 2; ++$i){
			...
			if($world->getBlockAt($x, $y, $z)->getTypeId() === BlockTypeIds::AIR){
				...
				foreach(self::SOIL_TYPES as $soil){
					if($soil === $block_below){
						$fallen_leaves = CustomiesBlockFactory::getInstance()->get("foxymc:fallen_leaves");
						if($fallen_leaves !== null){
							$world->setBlockAt($x, $y, $z, $fallen_leaves);
						}
						break;
					}
				}
