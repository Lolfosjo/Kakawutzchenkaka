		...
		for($i = 0; $i < 2; ++$i){
			...
			if($world->getBlockAt($x, $y, $z)->getTypeId() === BlockTypeIds::AIR){
				...
				foreach(self::SOIL_TYPES as $soil){
					if($soil === $block_below){
						$sukkulenten_block = CustomiesBlockFactory::getInstance()->get("foxymc:sukkulenten_pflanze");
						if($sukkulenten_block !== null){
							$world->setBlockAt($x, $y, $z, $sukkulenten_block);
						}
						break;
					}
				}
